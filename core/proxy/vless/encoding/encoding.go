...
// DialectAccepted reports whether a request version byte is one this inbound
// serves.
//
// An empty accepted set means {Version}, i.e. stock VLESS, which is what every
// inbound that names no dialect gets. This is the whole of the server-side
// check: the version byte is the first byte on the wire, so a client that does
// not speak a dialect the inbound accepts is turned away before its user id is
// even read.
func DialectAccepted(version byte, accepted []byte) bool {
	if len(accepted) == 0 {
...
//
// accepted lists the request version bytes -- the dialects -- this inbound
// serves. It is variadic so that a caller with no opinion keeps the stock
// behaviour of accepting version 0 only.
func DecodeRequestHeader(isfb bool, first *buf.Buffer, reader io.Reader, validator vless.Validator, accepted ...byte) ([]byte, *protocol.RequestHeader, *Addons, bool, error) {
	buffer := buf.StackNew()
	defer buffer.Release()
	...
	if !DialectAccepted(request.Version, accepted) {
		return nil, nil, nil, isfb, errors.New("invalid request version")
	}

	// A plain block, not a switch on the version: past the dialect check every
	// accepted version is parsed identically, and keeping the body indented
