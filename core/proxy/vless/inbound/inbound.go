...
type Handler struct {
	...
	// nil meaning stock VLESS only. See Config.dialects in config.proto.
	dialects  []byte
...
func New(ctx context.Context, config *Config, dc dns.Client, validator vless.Validator) (*Handler, error) {
	...
	// Narrowed to a byte here rather than at parse time so that a config built
	// by something other than infra/conf cannot smuggle a value past the range
	// check and silently accept a dialect nobody asked for.
	for _, d := range config.Dialects {
		if d > 255 {
			return nil, errors.New("VLESS inbound: dialect out of range: ", d).AtError()
		}
		handler.dialects = append(handler.dialects, byte(d))
	}
...
func (h *Handler) Process(ctx context.Context, network net.Network, connection stat.Connection, dispatch routing.Dispatcher) error {
	...
	if isfb && firstLen < 18 {
		...
		userSentID, request, requestAddons, isfb, err = encoding.DecodeRequestHeader(isfb, first, reader, h.validator, h.dialects...)
	}
