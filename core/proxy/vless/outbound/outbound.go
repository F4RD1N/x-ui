...
func (h *Handler) Process(ctx context.Context, link *transport.Link, dialer internet.Dialer) error {
	...
	request := &protocol.RequestHeader{
		// The dialect this account speaks rather than the package constant. It
		// is encoding.Version (0, stock VLESS) for every account that names no
		// dialect, so an unchanged config goes out on the wire it always did.
