...
func (c *VLessInboundConfig) Build() (proto.Message, error) {
	...
	// The dialects this inbound serves. Left empty it means stock VLESS only,
	// so an inbound that says nothing behaves as it always has.
	for _, d := range c.Dialects {
		if d > 255 {
			return nil, errors.New(`VLESS settings: "dialects" must each be 0-255, got `, d)
		}
	}
	config.Dialects = c.Dialects
...
func (c *VLessOutboundConfig) Build() (proto.Message, error) {
	...
	for _, rec := range c.Vnext {
		...
		for _, rawUser := range rec.Users {
			...
			// The dialect this outbound speaks. Zero, the value of every config
			// that names none, is stock VLESS. In the vnext style it arrives
			// through the json.Unmarshal above, because the generated Account
			// carries a "dialect" json tag; in the simplified style it is
			// copied from the settings above.
			if account.Dialect > 255 {
				return nil, errors.New(`VLESS clients: "dialect" must be 0-255, got `, account.Dialect)
			}
