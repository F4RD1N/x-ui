...
func (s *SubService) genVlessLink(inbound *model.Inbound, email string) string {
	...
	// The dialect a client built from this link has to speak. The inbound may
	// serve several; the link advertises the first, which is the one a fresh
	// client should use.
	if dialects, ok := settings["dialects"].([]any); ok && len(dialects) > 0 {
		if d, ok := dialects[0].(float64); ok {
			params["x-dialect"] = strconv.Itoa(int(d))
		}
	}
