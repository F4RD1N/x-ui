...
func (s *SubJsonService) genVless(inbound *model.Inbound, streamSettings json_util.RawMessage, client model.Client) json_util.RawMessage {
	...
	// The dialect this outbound must speak to be let in. The inbound may serve
	// several; the subscription hands out the first, matching the share link.
	if dialects, ok := inboundSettings["dialects"].([]any); ok && len(dialects) > 0 {
