...
class Inbound extends XrayCommonClass {
    ...
    genVLESSLink(address = '', port = this.port, forceTls, remark = '', clientId, flow) {
        ...
        // The dialect a client built from this link has to speak. The inbound
        // may serve several; the link advertises the first, which is the one a
        // fresh client should use.
        const dialects = this.settings.dialectList;
        if (dialects && dialects.length > 0) {
            params.set("x-dialect", dialects[0]);
        }
        switch (type) {
            case "tcp":
                const tcp = this.stream.tcp;
...
Inbound.VLESSSettings = class extends Inbound.Settings {
    constructor(
        ...
        // Held as the text the user typed ("4,5") rather than an array, so the
        // form can bind a plain input to it and nothing rewrites what they
        // entered. It becomes an array of numbers only on the way out, in
        // toJson().
        this.dialects = dialects;
    }

    // dialectList is the parsed form of this.dialects, used for the config and
    // for share links. Anything that is not a number is left out rather than
    // written into the config as null.
    get dialectList() {
        return String(this.dialects || "")
            .split(",")
            .map(d => d.trim())
            .filter(d => d !== "")
            .map(d => Number(d))
    ...
    toJson() {
        ...
        // An inbound naming no dialect must not carry the key at all: an empty
        // list and a missing one both mean stock VLESS to the core, but leaving
        // it out keeps configs identical to what they were before.
        const dialects = this.dialectList;
        if (dialects.length > 0) {
            json.dialects = dialects;
        }
