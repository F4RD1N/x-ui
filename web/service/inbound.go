...
func (s *InboundService) AddInbound(inbound *model.Inbound) (*model.Inbound, bool, error) {
	// A certificate or key in the stream settings may be given as a link
	// instead of a path. Fetch it now and store where it landed, so the core
	// only ever opens local files.
	materialized, err := MaterializeStreamCerts(inbound.StreamSettings)
	if err != nil {
		return inbound, false, err
	}
	inbound.StreamSettings = materialized
