...
var defaultValueMap = map[string]string{
	...
	// Fetching the Xray config template from a URL on a timer. An empty URL or
...
// GetXrayConfigUrl returns the URL the Xray config template is fetched from,
// empty when the timed fetch is off.
func (s *SettingService) GetXrayConfigUrl() (string, error) {
	return s.getString("xrayConfigUrl")
}

func (s *SettingService) SetXrayConfigUrl(url string) error {
	return s.setString("xrayConfigUrl", url)
}

// GetXrayConfigInterval returns how many minutes pass between fetches of the
// Xray config template. Zero means the timed fetch is off.
func (s *SettingService) GetXrayConfigInterval() (int, error) {
	return s.getInt("xrayConfigInterval")
}
...
func (s *SettingService) UpdateAllSetting(allSetting *entity.AllSetting) error {
	...
	// A certificate field may hold a link instead of a path. Fetch it now and
	// store where it landed, so the panel only ever reads local files.
	for _, field := range []*string{&allSetting.WebCertFile, &allSetting.WebKeyFile} {
		local, err := DownloadCert(*field)
		if err != nil {
			return err
		}
		*field = local
